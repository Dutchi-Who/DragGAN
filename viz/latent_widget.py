...
class LatentWidget:
    def __init__(self, viz, path=""):
        ...
        self.lr = 0.001
        self.w_path = path
        self.w_load = None if self.w_path == "" else torch.load(self.w_path)
        self.defer_frames = 0
    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('Load latent', width=viz.button_w,
                                      enabled=(self.disabled_time == 0 and 'image' in viz.result)):
